...
describe('requestCodeSchema', () => {
  ...
  it('should provide helpful error messages for long phone numbers', () => {
    const result = requestCodeSchema.safeParse({ phoneNumber: '123456789012345678901' });
    expect(result.success).toBe(false);
    if (!result.success) {
...
describe('verifyCodeSchema', () => {
  ...
  it('should provide helpful error messages for invalid codes', () => {
    const result = verifyCodeSchema.safeParse({ phoneNumber: '1234567890', code: '12345' });
    expect(result.success).toBe(false);
    if (!result.success) {
...
describe('completeProfileSchema', () => {
  ...
  it('should provide helpful error messages for long display names', () => {
    const result = completeProfileSchema.safeParse({ displayName: 'a'.repeat(51) });
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  it('should reject missing displayName field', () => {
    const result = completeProfileSchema.safeParse({ timezone: 'America/New_York' });
    expect(result.success).toBe(false);
  });
