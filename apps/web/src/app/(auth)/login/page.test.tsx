...
describe('LoginPage', () => {
  ...
  it('renders the login form', () => {
    ...
    expect(
      screen.getByText('Enter your phone number to sign in or create an account')
    ).toBeDefined();
  ...
  it('displays validation error for phone number that is too short', async () => {
    ...
    await waitFor(
      () => {
        const errorMessage = screen.queryByText(/phone number must be at least 10 characters/i);
  ...
  it('displays validation error for phone number that is too long', async () => {
    ...
    await waitFor(() => {
      const errorMessage = screen.queryByText('Phone number must not exceed 20 characters');
  ...
  it('displays rate limit error', async () => {
    ...
    await waitFor(() => {
      const errorMessage = screen.queryByText('Too many requests. Please try again later.');
      expect(errorMessage).toBeTruthy();
    });
  });
  ...
  it('shows terms and privacy policy disclaimer', () => {
    ...
    expect(
      screen.getByText('By continuing, you agree to our Terms of Service and Privacy Policy')
    ).toBeDefined();
  });
});
