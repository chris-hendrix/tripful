...
describe('permissions.service', () => {
  ...
  describe('isOrganizer', () => {
    it('should return true for trip creator', async () => {
      const result = await permissionsService.isOrganizer(testCreatorId, testTripId);
      expect(result).toBe(true);
    });

    it('should return true for co-organizer (member with status=going)', async () => {
      const result = await permissionsService.isOrganizer(testCoOrganizerId, testTripId);
      expect(result).toBe(true);
    });

    it('should return false for regular member (status!=going)', async () => {
      const result = await permissionsService.isOrganizer(testMemberId, testTripId);
      expect(result).toBe(false);
    });

    it('should return false for non-member', async () => {
      const result = await permissionsService.isOrganizer(testNonMemberId, testTripId);
      expect(result).toBe(false);
    });

    it('should return false for non-existent trip', async () => {
      const result = await permissionsService.isOrganizer(testCreatorId, '00000000-0000-0000-0000-000000000000');
      expect(result).toBe(false);
    });

    it('should return false for non-existent user', async () => {
      const result = await permissionsService.isOrganizer('00000000-0000-0000-0000-000000000000', testTripId);
      expect(result).toBe(false);
    });
  });
  ...
  describe('isMember', () => {
    it('should return true for co-organizer (who is also a member)', async () => {
      const result = await permissionsService.isMember(testCoOrganizerId, testTripId);
      expect(result).toBe(true);
    });

    it('should return true for regular member', async () => {
      const result = await permissionsService.isMember(testMemberId, testTripId);
      expect(result).toBe(true);
    });

    it('should return false for non-member', async () => {
      const result = await permissionsService.isMember(testNonMemberId, testTripId);
      expect(result).toBe(false);
    });
    ...
    it('should return false for trip creator if they are not also a member', async () => {
      ...
      const result = await permissionsService.isMember(testCreatorId, testTripId);
      expect(result).toBe(false);
    });

    it('should return false for non-existent trip', async () => {
      const result = await permissionsService.isMember(testMemberId, '00000000-0000-0000-0000-000000000000');
      expect(result).toBe(false);
    });

    it('should return false for non-existent user', async () => {
      const result = await permissionsService.isMember('00000000-0000-0000-0000-000000000000', testTripId);
      expect(result).toBe(false);
    });
  });
  ...
  describe('canEditTrip', () => {
    it('should return true for trip creator', async () => {
      const result = await permissionsService.canEditTrip(testCreatorId, testTripId);
      expect(result).toBe(true);
    });

    it('should return true for co-organizer', async () => {
      const result = await permissionsService.canEditTrip(testCoOrganizerId, testTripId);
      expect(result).toBe(true);
    });

    it('should return false for regular member', async () => {
      const result = await permissionsService.canEditTrip(testMemberId, testTripId);
      expect(result).toBe(false);
    });

    it('should return false for non-member', async () => {
      const result = await permissionsService.canEditTrip(testNonMemberId, testTripId);
      expect(result).toBe(false);
    });
  });
  ...
  describe('canDeleteTrip', () => {
    it('should return true for trip creator', async () => {
      const result = await permissionsService.canDeleteTrip(testCreatorId, testTripId);
      expect(result).toBe(true);
    });

    it('should return true for co-organizer', async () => {
      const result = await permissionsService.canDeleteTrip(testCoOrganizerId, testTripId);
      expect(result).toBe(true);
    });

    it('should return false for regular member', async () => {
      const result = await permissionsService.canDeleteTrip(testMemberId, testTripId);
      expect(result).toBe(false);
    });

    it('should return false for non-member', async () => {
      const result = await permissionsService.canDeleteTrip(testNonMemberId, testTripId);
      expect(result).toBe(false);
    });
  });
  ...
  describe('canManageCoOrganizers', () => {
    it('should return true for trip creator', async () => {
      const result = await permissionsService.canManageCoOrganizers(testCreatorId, testTripId);
      expect(result).toBe(true);
    });

    it('should return true for co-organizer', async () => {
      const result = await permissionsService.canManageCoOrganizers(testCoOrganizerId, testTripId);
      expect(result).toBe(true);
    });

    it('should return false for regular member', async () => {
      const result = await permissionsService.canManageCoOrganizers(testMemberId, testTripId);
      expect(result).toBe(false);
    });

    it('should return false for non-member', async () => {
      const result = await permissionsService.canManageCoOrganizers(testNonMemberId, testTripId);
      expect(result).toBe(false);
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle member status changes correctly', async () => {
      ...
      let result = await permissionsService.isOrganizer(testMemberId, testTripId);
      expect(result).toBe(true);
    ...
    it('should handle creator also being a member', async () => {
      ...
      const isOrganizerResult = await permissionsService.isOrganizer(testCreatorId, testTripId);
      expect(isOrganizerResult).toBe(true);

      // Should also be a member now
      const isMemberResult = await permissionsService.isMember(testCreatorId, testTripId);
    ...
    it('should handle multiple co-organizers', async () => {
      ...
      const result = await permissionsService.isOrganizer(anotherUserId, testTripId);
      expect(result).toBe(true);
