...
export interface ITripService {
  ...
  addCoOrganizers(tripId: string, userId: string, phoneNumbers: string[]): Promise<void>;
  removeCoOrganizer(tripId: string, userId: string, coOrgUserId: string): Promise<void>;
...
const createTripMutation = useMutation({
  ...
  onMutate: async (newTrip) => {
    ...
    queryClient.setQueryData(['trips'], (old) => [...old, {
      ...newTrip,
      id: 'temp-id',
      isOptimistic: true
    }]);
...
const { data: trip, error } = useQuery({
  queryKey: ['trips', id],
  queryFn: () => fetch(`/api/trips/${id}`).then(r => {
    if (r.status === 404) throw new Error('Trip not found');
    if (r.status === 403) throw new Error('You do not have access to this trip');
    return r.json();
  })
});

if (error?.message === 'Trip not found' || error?.message === 'You do not have access to this trip') {
...
export const addCoOrganizerSchema = z.object({
  phoneNumbers: z.array(z.string().min(10).max(20))
...
test('non-organizer cannot edit trip', async ({ page }) => {
  ...
  // Verify no access
  await expect(page.locator('text=You do not have access to this trip')).toBeVisible();
